...
class bipartite_dataset(Dataset):  
    ...
    def negs_gen_EP(self,epoch):
        ...
        for j in set(self.train['userId'].values):
            pos=self.train[self.train['userId']==j]['movieId'].values-1
            neg = np.setdiff1d(self.tot,pos) # contains item id that node j doesn't rate
            ...
            # With each user, there is 40 item id that user doesn't rate
            prog.update(1)
        prog.close()
    ...
    def __getitem__(self,idx):
        ...
        v = self.edge_2[idx]
        w = self.edge_3[idx]
        negs = self.edge_4[idx] # batch_size 40, each data point presents user id and 40 negative item
        return u,v,w,negs
...
def gen_top_k(data_class, r_hat, K=300):
    ...
    no_items = all_items - tot_items; # item not in both train and test set
    ...
    # for each user, choose 300 item in testset with highest rating score
    _, reco = torch.topk(r_hat,K) # 6040 300
    reco = reco.numpy()    
