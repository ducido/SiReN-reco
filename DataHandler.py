...
class DataHandler:
    ...
    def LoadData(self):
        print(self.trnfile, self.tstfile)
        trnMat = self.loadOneFile(self.trnfile)
        tstMat = self.loadOneFile(self.tstfile)
        args.user, args.item = trnMat.shape
        self.num_u, self.num_v = trnMat.shape

        train = pd.DataFrame({
            'userId': trnMat.row + 1,    # Row indices as user IDs
            'movieId': trnMat.col + 1,
            'rating': 1  
        })


        test = pd.DataFrame({
            'userId': tstMat.row + 1,    # Row indices as user IDs
            'movieId': tstMat.col + 1,
            'rating': 1
        })
...
class CustomDataHandler(DataHandler):
    def __init__(self):
        super(CustomDataHandler, self).__init__()
        predir = "./Datasets/sparse_yelp/"
        self.trnfile = predir + 'trnMat.pkl'
        self.tstfile = predir + 'tstMat.pkl'
